...
auto B_PLUS_TREE_LEAF_PAGE_TYPE::SpInsert(BPlusTreeLeafPage &page, int index, const KeyType &key,
  ...
  memcpy(reinterpret_cast<void *>(page.array_), reinterpret_cast<void *>(mid_array + mid_index),
         sizeof(MappingType) * (cursize - mid_index));
  SetSize(mid_index);
...
INDEX_TEMPLATE_ARGUMENTS
auto B_PLUS_TREE_LEAF_PAGE_TYPE::MapAt(int index) -> MappingType & { return *(array_ + index); }
