...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if (func_name == "lower") {
    return std::make_shared<StringExpression>(std::move(args[0]), bustub::StringExpressionType::Lower);
  }
  if (func_name == "upper") {
    return std::make_shared<StringExpression>(std::move(args[0]), bustub::StringExpressionType::Upper);
  }
  throw Exception(fmt::format("func call {} not supported in planner yet", func_name));
}
